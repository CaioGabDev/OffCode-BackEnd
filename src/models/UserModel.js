...
const createUsers = async (nome, username, email, senha, tipo_conta, foto_perfil, descricao, especializacoes) => {
    const result = await pool.query("INSERT INTO usuarios (name, email, password) VALUES ($1, $2, $3) RETURNING *", [nome, username, email, senha, tipo_conta, foto_perfil, descricao, especializacoes]);
    return result.rows[0];
};
